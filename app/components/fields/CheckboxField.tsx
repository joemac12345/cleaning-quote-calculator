      ...
      <div className="space-y-3">
        {options.map((option) => {
          ...
          return (
            <div
              key={option.value}
              className="flex items-start sm:items-center justify-between border border-gray-200 rounded-lg p-3 sm:p-4 gap-2"
            >
              <div className="flex items-start sm:items-center gap-2 sm:gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {option.price && <p className="text-[#48546A] font-semibold text-sm sm:text-base">£{option.price}</p>}
                  {showTime && option.time && <p className="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1">{option.time} min</p>}
                </div>
              </div>
              <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                ...
                <span className="w-5 sm:w-6 text-center text-base sm:text-lg font-semibold text-gray-900">{count}</span>
                <button
